...
router.get('/', async (_req, res) => {
  const { rows } = await pool.query('SELECT * FROM servicios_descuento ORDER BY codigo');
  res.json(rows);
});
...
router.post('/', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO servicios_descuento (codigo, nombre) VALUES ($1, $2) RETURNING *`,
...
router.patch('/:codigo', async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE servicios_descuento SET ${set.join(', ')} WHERE codigo=$${params.length} RETURNING *`,
    params
  );
